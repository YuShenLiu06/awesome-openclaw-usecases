...
| [自主游戏开发流水线](usecases/autonomous-game-dev-pipeline.md) | 教育游戏开发的完整生命周期管理：从积压选择到实现、注册、文档和 git 提交。强制执行"Bug 优先"政策。 |
...
| 名称 | 描述 |
|------|-------------|
| [n8n 工作流程编排](usecases/n8n-workflow-orchestration.md) | 通过 webhook 将 API 调用委托给 n8n 工作流程 —— 智能体从不接触凭据，每个集成都可视且可锁定。 |
| [自愈家庭服务器](usecases/self-healing-home-server.md) | 运行一个始终在线的基础设施智能体，具有 SSH 访问、自动化 cron 作业，以及跨家庭网络的自愈能力。 |
...
| [Todoist 任务管理器](usecases/todoist-task-manager.md) | 通过将推理和进度日志同步到 Todoist，最大化智能体透明度。 |
...
| [把 OpenClaw 当桌面 Cowork](usecases/aionui-cowork-desktop.md) | 在桌面 Cowork 应用中使用 OpenClaw：统一界面、多智能体、MCP 一次配置，支持 WebUI/Telegram/Lark/钉钉。当 OpenClaw 无法连接时，可通过 Telegram 或 WebUI 远程使用内置的 OpenClaw 部署专家进行安装、检测与修复。 |
...
| [市场研究与产品工厂](usecases/market-research-product-factory.md) | 使用 Last 30 Days 技能从 Reddit 和 X 中挖掘真正的痛点，然后让 OpenClaw 构建解决它们的 MVP。 |
